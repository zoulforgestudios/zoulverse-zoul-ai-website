  ...
  const processUserInput = async (text: string) => {
    ...
    try {
      ...
      if (response) {
        ...
        setCurrentResponse(response); // Show current response
        await speak(response);
        
        // Clear response after speaking
        setTimeout(() => setCurrentResponse(''), 5000);
      }
  ...
  return (
    <div className="zoul-app">
      ...
      <div className="container">
        ...
        {/* Bottom Text Display - Shows what you're saying and Zoul's response */}
        <div className={`bottom-text-display ${(transcript || currentResponse) ? 'active' : ''}`}>
          {transcript && (
            <div className="bottom-text user-text">
              <span className="label">You:</span> {transcript}
            </div>
          )}
          {currentResponse && !transcript && (
            <div className="bottom-text zoul-text">
              <span className="label">Zoul:</span> {currentResponse}
            </div>
          )}
        </div>
